...
export class NotificationBellComponent implements OnInit, OnDestroy {
  ...
  get notificationSections() {
    return [
      {
        label: 'Today',
        items: this.notifications.filter((notification) =>
          this.isToday(notification.created_at),
        ),
      },
      {
        label: 'Yesterday',
        items: this.notifications.filter(
          (notification) => !this.isToday(notification.created_at),
        ),
      },
    ];
  }
  ...
  getNotificationTitle(notification: AppNotification) {
    ...
    if (type === 'booking') return 'New Booking Request';
  ...
  getNotificationDescription(notification: AppNotification) {
    return (
      notification.message
        .replace(this.getNotificationHighlight(notification), '')

...
<body>
    ...
    if (strpos($request, 'get_monthly_bookings_range') !== false) {
        $result = $get->get_monthly_bookings_range();
        echo json_encode($result);
        exit();
    }

    if (strpos($request, 'get_revenue_by_date_range') !== false) {
        $result = $get->get_revenue_by_date_range();
        echo json_encode($result);
        exit();
    }

    if (strpos($request, 'get_daily_revenue_range') !== false) {
        $result = $get->get_daily_revenue_range();
        echo json_encode($result);
        exit();
    }

    if (strpos($request, 'get_weekly_revenue_range') !== false) {
        $result = $get->get_weekly_revenue_range();
        echo json_encode($result);
        exit();
    }

    if (strpos($request, 'get_report_summary') !== false) {
        $result = $get->get_report_summary();
        echo json_encode($result);
        exit();
    }

...

{
    ...
    public function register_admin($data) {
        ...
        try {
            ...
            if ($statement->rowCount() > 0) {
                // Send email
                $this->send_admin_credentials_email($data->email, $data->first_name, $data->password, $data->admin_id);
    ...
    public function resend_admin_password($data) {
        if (empty($data->id)) {
        ...
        try {
            ...
            if ($updateStmt->rowCount() > 0 || $updateStmt->errorCode() === '00000') {
                // Send email
                $emailSent = $this->send_admin_credentials_email($admin['email'], $admin['first_name'], $newPassword, $admin['admin_id']);
            ...
            error_log("Resend admin password error: " . $e->getMessage());
            return $this->sendPayload(null, "failed", "Database error occurred.", 500);
        }
    }

    private function send_admin_credentials_email($email, $firstName, $password, $adminId) {
        ...
        $htmlContent = "
            ...
            <p>Your admin account has been created or your password has been reset. Here are your credentials:</p>
            <ul>
                <li><strong>Admin ID:</strong> " . htmlspecialchars($adminId) . "</li>
